...
</ul><p>A script may determine which of these methods is available through <strong>mwf.touch.geolocation.getType()</strong> where 1 represents HTML 5, 2 represents Google Gears, and 0 represents unavailable. Similarly, one may merely determine if geolocation is supported through <strong>mwf.touch.geolocation.isSupported()</strong>. Although this library supports two different methods with different underlying standards, it abstracts them into a common interface so, besides determining which library is supported, it does not matter which the device supports.</p>
...
<pre><code>mwf.touch.geolocation.setHighAccuracy(false);</code></pre>
...
<pre><code>mwf.touch.geolocation.getCurrentPosition(onSuccessCallback, onFailureCallback).</code></pre>

<p>Consider the following example:</p>

<pre><code>mwf.touch.geolocation.getCurrentPosition(
...
<p>On a page, more than one script may seek to leverage geolocation information, or it may need to on a recurring basis. However, such scripts may not always seek to go through the retrieval process. Therefore, in the event that <strong>mwf.touch.geolocation.getPosition(...)</strong> is called more than once, if the geolocation data has already been retrieved, this library will instead immediately return it to the callback. </p>
...
<pre><code>mwf.touch.geolocation.setTimeout(ms);</code></pre>
...
<pre><code>mwf.touch.geolocation.watchPosition(onSuccessCallback, onFailureCallback)</code></pre>

<p><code>watchPosition</code> returns a unique ID number that can be passed to <code>clearWatch</code> to discontinue polling:</p>

<pre><code>mwf.touch.geolocation.clearWatch(watchID)</code></pre>
...
<pre><code>var watchID = mwf.touch.geolocation.watchPosition(function(pos) {
	if(pos['accuracy'] &lt;= 30) {
